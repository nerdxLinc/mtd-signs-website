...
6. Onboard `mtdsigns.com` to Cloudflare Email Service and verify `mtdsigns@gmail.com` under **Email Routing > Destination Addresses**. Create an account-scoped API token with only **Email Sending: Edit** permission. In the Pages project's **Settings > Variables and Secrets**, add `CLOUDFLARE_ACCOUNT_ID` as a production variable and `CLOUDFLARE_EMAIL_API_TOKEN` as an encrypted production secret, then redeploy. Pages does not expose a direct `send_email` binding, so the contact Function uses Cloudflare Email Service's REST API. Never commit the token. The recipient remains fixed as `mtdsigns@gmail.com` and the sender as `website@mtdsigns.com`. A contact inquiry is always saved to D1 first; an email failure is recorded on that inquiry and never discards the lead.
...
8. The middleware validates Cloudflare Access's signed `Cf-Access-Jwt-Assertion`, extracts the authenticated email, and passes that verified identity to every admin API endpoint. The current Access team domain and application audience are built in; they can be overridden with `CF_ACCESS_TEAM_DOMAIN` and `CF_ACCESS_AUD` if the Access application is ever recreated. Hiding the admin link alone is never used as protection.
